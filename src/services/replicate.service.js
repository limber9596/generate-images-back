...
async function generateImageFromBoceto_s(bocetoPath, prompt) {
  const buffer_img = await fs.promises.readFile(bocetoPath);
  const image = await Jimp.read(buffer_img); // o usar Jimp.create() si quieres una imagen vacía
  ...
  // Ejecutar el modelo con versión específica
  ...
  const fileName = "output_img2img.png";
